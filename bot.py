...
async def process_withdraw(update, context, uid, service, user_input):
    ...
    # ========== MODIFIED: added service_tag ==========
    r = create_withdrawal_atomic(
        uid,
        req_pts,
        username,
        full_name,
        ...
        service_tag=service.get("tag", ""),   # <-- ADDED
        required_referrals=service.get("required_referrals", 0),
...
async def payout_approve(update, context, serial_no):
    ...
    # ========== NEW: Generate redeem code ==========
    code = uuid.uuid4().hex[:12]
    get_collection("redeem_codes").insert_one({
        "code": code,
        "user_id": r["user_id"],
    ...
    await safe_send(context.bot, r["user_id"],
                    ...
                    f"Service: {r.get('service_name', 'Telegram Account')}",
...
async def handle_admin_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ========== MODIFIED: service creation now asks for tag ==========
    elif awaiting == "service_input_label":
        label = text if text.lower() != "none" else ""
        context.user_data["service_input_label"] = label
        context.user_data["awaiting"] = "service_tag"
        await update.message.reply_text("🏷️ Enter a tag for redemption (e.g., 'usa', 'india', 'spam'):")
        return
    elif awaiting == "service_tag":
        tag = text.strip().lower() or context.user_data.get("service_name", "").lower().replace(" ", "_")
        get_collection("ui_services").insert_one({
            ...
            "input_label": context.user_data.get("service_input_label", ""),
            "tag": tag,
            "active": True,
            "created_date": datetime.now()
        })
        context.user_data.clear()
        await update.message.reply_text("✅ sᴇʀᴠɪᴄᴇ ᴀᴅᴅᴇᴅ!", reply_markup=create_admin_keyboard())
    # ================================================================
